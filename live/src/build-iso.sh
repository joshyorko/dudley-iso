...
#     LiveOS/squashfs.img       squashfs of the full Dudley live rootfs (single-arch)
...
#   dmsquash-live: scans for LABEL=DUDLEY_LIVE → mounts ISO → squashfs → overlayfs
#
...
LIVE_TITLE="${LIVE_TITLE:-Dudley Installer}"
LABEL="DUDLEY_LIVE"
MULTI_ARCH=false
...
if [[ "${MULTI_ARCH}" == "true" ]]; then
    ...
    for spec in "${ARCH_SPECS[@]}"; do
        ...
        cat > "${ESP_STAGING}/loader/entries/dudley-live-${arch}.conf" << EOF
title   ${LIVE_TITLE} (${arch})
linux   /images/pxeboot/${arch}/vmlinuz
initrd  /images/pxeboot/${arch}/initrd.img
options root=live:LABEL=DUDLEY_LIVE rd.live.image rd.live.overlay.overlayfs=1 rd.live.squashimg=squashfs-${arch}.img enforcing=0 nvidia-drm.modeset=1 console=${local_console},115200n8
EOF
...
timeout 5
default dudley-live-${FIRST_ARCH}.conf
EOF
...
menuentry "${LIVE_TITLE} (${arch})" {
    linux /images/pxeboot/${arch}/vmlinuz root=live:LABEL=DUDLEY_LIVE rd.live.image rd.live.overlay.overlayfs=1 rd.live.squashimg=squashfs-${arch}.img enforcing=0 nvidia-drm.modeset=1 console=${local_console},115200n8 rd.dudley.isofile=\${iso_path}
    initrd /images/pxeboot/${arch}/initrd.img
}
EOF
        done
...
EOF
    ...
    cat > "${ESP_STAGING}/loader/entries/dudley-live.conf" << EOF
title   ${LIVE_TITLE}
linux   /images/pxeboot/vmlinuz
initrd  /images/pxeboot/initrd.img
options root=live:LABEL=DUDLEY_LIVE rd.live.image rd.live.overlay.overlayfs=1 enforcing=0 nvidia-drm.modeset=1 console=ttyS0,115200n8 console=ttyAMA0,115200n8
EOF
...
menuentry "${LIVE_TITLE}" {
    linux /images/pxeboot/vmlinuz root=live:LABEL=DUDLEY_LIVE rd.live.image rd.live.overlay.overlayfs=1 enforcing=0 nvidia-drm.modeset=1 console=ttyS0,115200n8 console=ttyAMA0,115200n8 rd.dudley.isofile=\${iso_path}
    initrd /images/pxeboot/initrd.img
}
EOF
...
if [[ "${MULTI_ARCH}" == "true" ]]; then
    for spec in "${ARCH_SPECS[@]}"; do
        ...
        mcopy -i "${ESP_IMG}" "${ESP_STAGING}/loader/entries/dudley-live-${arch}.conf" "::/loader/entries/dudley-live-${arch}.conf"
    ...
    mcopy -i "${ESP_IMG}" "${ESP_STAGING}/loader/entries/dudley-live.conf" "::/loader/entries/dudley-live.conf"
fi
...
#   Keep the reserved partition gap; some firmware rejects tightly packed ESPs.
xorriso -as mkisofs \
    -iso-level 3 \
    -r \
    -J --joliet-long \
