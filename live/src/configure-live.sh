#!/usr/bin/bash
# Live-environment setup for the Dudley ISO installer image.
#
...
# Suppress the inherited GNOME welcome dialog on first login.
...
favorite-apps=['dudley-installer.desktop', 'org.mozilla.firefox.desktop', 'org.gnome.Nautilus.desktop', 'org.gnome.Console.desktop']
...
# Prints DUDLEY_LIVE_READY to the serial console after display-manager.service
...
[Service]
Type=oneshot
ExecStart=/bin/echo DUDLEY_LIVE_READY
StandardOutput=tty
TTYPath=/dev/ttyS0
...
for size in 16 24 32 48 64 128 256 512; do
  install -Dm644 "$SCRIPT_DIR/icons/hicolor/${size}x${size}/apps/dudley.png" \
    "/usr/share/icons/hicolor/${size}x${size}/apps/dudley.png"
done
# Symlink 512×512 to pixmaps for compatibility
install -Dm644 "$SCRIPT_DIR/icons/hicolor/512x512/apps/dudley.png" /usr/share/pixmaps/dudley.png
...
install -Dm644 "$SCRIPT_DIR/images/dudley-raptor.png" /usr/share/bootc-installer/images/dudley-raptor.png
...
if [[ -f "$VARIANT_DIR/base_imgref" ]]; then
    ...
    BASE_IMGREF="ghcr.io/joshyorko/dudley-os:dakota"
fi
if [[ -f "$VARIANT_DIR/nvidia_imgref" ]]; then
    NVIDIA_IMGREF=$(cat "$VARIANT_DIR/nvidia_imgref")
else
    NVIDIA_IMGREF="ghcr.io/joshyorko/dudley-os:dakota-nvidia"
fi
...
# entry makes it visible and pinnable as 'dudley-installer.desktop'.
mkdir -p /usr/share/applications
cat > /usr/share/applications/dudley-installer.desktop << DTEOF
[Desktop Entry]
Name=Dudley Installer
Comment=Install Dudley to your computer
Exec=flatpak run --env=BOOTC_CUSTOM_RECIPE=/run/host/etc/bootc-installer/recipe.json ${INSTALLER_APP_ID}
Icon=dudley
Type=Application
Categories=System;
